...
# document history
hist <- read_excel("R:/tracking_sheets/wave_report_tracker.xlsx") %>% 
  filter(Depl_ID == depl_id) %>% 
...
p1 <- dat %>% 
  ...
  wv_plot_histogram(
    ...
    pal = brewer.pal(3, "Dark2")[2]
  ) +
  scale_x_continuous("Significant Height (m)") +
...
colors_wv <- brewer.pal(length(breaks_wv), "Oranges")
...
p <- p$plot +
  latticeExtra::layer(
    lattice::ltext(0, dir_label_wv, "N", cex = 0.75, col = "darkgrey")
  ) +
  latticeExtra::layer(
    lattice::ltext(dir_label_wv, 0, "E", cex = 0.75, col = "darkgrey")
  ) +
  latticeExtra::layer(
    lattice::ltext(-dir_label_wv, 0, "W", cex = 0.75, col = "darkgrey")
  ) +
  latticeExtra::layer(
    lattice::ltext(0, -dir_label_wv, "S", cex = 0.75, col = "darkgrey")
  )

p
 # openair::windRose(
 #    dat_wave,
 #    ws = "significant_height_m", wd = "to_direction_degree",
 #   breaks = breaks_wv,
 #    #cols = speed_cols,
 #    paddle = FALSE,
 #    auto.text = FALSE,
...
```
Figure `r k`: Wave height and the direction the wave is travelling to.
...
p_curr <- p_curr$plot +
  latticeExtra::layer(
    lattice::ltext(0, dir_label_curr, "N", cex = 0.75, col = "darkgrey")
  ) +
  latticeExtra::layer(
    lattice::ltext(dir_label_curr, 0, "E", cex = 0.75, col = "darkgrey")
  ) +
  latticeExtra::layer(
    lattice::ltext(-dir_label_curr, 0, "W", cex = 0.75, col = "darkgrey")
  ) +
  latticeExtra::layer(
    lattice::ltext(0, -dir_label_curr, "S", cex = 0.75, col = "darkgrey")
  )
...
Figure `r k`: Current speed and direction for three near-surface bins. Top: Current rose (North is up). Bottom: Current speed distribution. The number of observations is noted above each bar.
